...
@section('title', 'Add Availability for Doctor')
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-lg shadow-lg p-6">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Add Availability Slot</h2>
                <p class="text-sm text-gray-600 mt-1">For Dr. {{ $doctor->name }}</p>
            </div>
            <a href="{{ route('doctors.show', ['doctor' => $doctor->id, 'tab' => 'availability']) }}" class="text-gray-600 hover:text-gray-900">
        ...
        <form action="{{ route('admin.doctors.availability.store', $doctor) }}" method="POST">
            @csrf
            ...
            <div class="mb-6">
                ...
                <div class="flex space-x-4">
                    <label class="flex items-center">
                        <input type="radio" name="availability_type" value="weekly" checked onchange="toggleAvailabilityType()" class="mr-2">
                        <span>Weekly Recurring</span>
                    </label>
                    <label class="flex items-center">
                        <input type="radio" name="availability_type" value="specific" onchange="toggleAvailabilityType()" class="mr-2">
            ...
            <div id="weekly_fields" class="mb-4">
            ...
            <div id="specific_fields" class="mb-4 hidden">
                <label for="specific_date" class="block text-sm font-medium text-gray-700 mb-2">Specific Date</label>
                <input type="date" name="specific_date" id="specific_date" value="{{ old('specific_date') }}" min="{{ date('Y-m-d') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500">
                @error('specific_date')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="time" name="start_time" id="start_time" value="{{ old('start_time') }}" required class="w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500">
                    @error('start_time')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="time" name="end_time" id="end_time" value="{{ old('end_time') }}" required class="w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500">
                    @error('end_time')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="slot_duration" id="slot_duration" value="{{ old('slot_duration', 30) }}" min="15" max="120" step="15" required class="w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500">
                ...
                <div>
                    ...
                    <input type="number" name="break_duration" id="break_duration" value="{{ old('break_duration', 0) }}" min="0" max="60" step="5" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500">
            ...
            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="is_available" value="1" {{ old('is_available', true) ? 'checked' : '' }} class="rounded border-gray-300 text-green-600 focus:ring-green-500">
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" id="notes" rows="3" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-end space-x-4">
                <a href="{{ route('doctors.show', ['doctor' => $doctor->id, 'tab' => 'availability']) }}" class="px-6 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
                    Cancel
                </a>
