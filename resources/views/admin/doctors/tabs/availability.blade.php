<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            <h3 class="text-lg font-semibold text-gray-900">Doctor Availability Management</h3>
            <p class="text-sm text-gray-600 mt-1">View and manage availability slots for Dr. {{ $doctor->name }}</p>
        </div>
        <a href="{{ route('admin.doctors.availability.create', $doctor) }}" class="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 flex items-center">
    ...
    <div class="bg-white border rounded-lg p-6">
        ...
        @if($availabilities->isEmpty())
            <div class="text-center py-8 text-gray-500">
                ...
                <p>No weekly availability set for this doctor.</p>
            </div>
        @else
            <div class="space-y-4">
                @foreach($days as $day)
                                ...
                                @foreach($daySlots as $slot)
                                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg border {{ $slot->is_available ? 'border-green-200' : 'border-red-200' }}">
                                        <div class="flex-1">
                                            ...
                                            @if($slot->notes)
                                                <div class="text-xs text-gray-400 mt-1 italic">{{ $slot->notes }}</div>
                                            @endif
                                        </div>
                                        <div class="flex items-center space-x-2 ml-4">
                                            <a href="{{ route('admin.doctors.availability.edit', ['doctor' => $doctor, 'availability' => $slot]) }}" class="text-blue-600 hover:text-blue-900" title="Edit">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                                </svg>
                                            </a>
                                            <form action="{{ route('admin.doctors.availability.destroy', ['doctor' => $doctor, 'availability' => $slot]) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this availability slot?');">
                                                @csrf
                                                @method('DELETE')
                        ...
                        @foreach($specificAvailabilities as $slot)
                            <tr>
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('admin.doctors.availability.edit', ['doctor' => $doctor, 'availability' => $slot]) }}" class="text-blue-600 hover:text-blue-900 mr-3">Edit</a>
                                    <form action="{{ route('admin.doctors.availability.destroy', ['doctor' => $doctor, 'availability' => $slot]) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure?');">
                                        @csrf
                                        @method('DELETE')
